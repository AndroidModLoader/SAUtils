...
void SAUtils::MarkEntityAsNotNeeded(CEntity* ent)
{
    ...
    switch(ent->m_nType)
    {
        ...
        case ENTITY_TYPE_PED:
            if(*ms_pPedPool != NULL && (*ms_pPedPool)->IsFromObjectArray((CPed*)ent)) ScriptCommand(&scm_MARK_CHAR_AS_NO_LONGER_NEEDED, (*ms_pPedPool)->GetRef((CPed*)ent));
            return;
        case ENTITY_TYPE_OBJECT:
            if(*ms_pObjectPool != NULL && (*ms_pObjectPool)->IsFromObjectArray((CObject*)ent)) ScriptCommand(&scm_MARK_OBJECT_AS_NO_LONGER_NEEDED, (*ms_pObjectPool)->GetRef((CObject*)ent));
            return;
        case ENTITY_TYPE_VEHICLE:
            if(*ms_pVehiclePool != NULL && (*ms_pVehiclePool)->IsFromObjectArray((CVehicle*)ent)) ScriptCommand(&scm_MARK_CAR_AS_NO_LONGER_NEEDED, (*ms_pVehiclePool)->GetRef((CVehicle*)ent));
            return;
    }
}
...
void SAUtils::PutPedInVehicle(CPed* ped, CVehicle* vehicle, int seat)
{
    ...
    if(!*ms_pPedPool || *ms_pVehiclePool ||
       !(*ms_pPedPool)->IsFromObjectArray(ped) ||
       !(*ms_pVehiclePool)->IsFromObjectArray(vehicle)) return;
